...
strart_local_redis() {
  if [ -z "${REDIS_HOST}" ]; then  ## use external redis
    if [ -n "$LOCAL_REDIS" ] || [ -n "$INSTALL_REDIS" ]; then
      ...
      redis-server --daemonize yes --save "" --appendonly no --bind 0.0.0.0 --protected-mode no --requirepass "aslkdjkkjadwkljdaslkjdwqklqajd39821ikdjal" --tls-port 6379 --port 0 --tls-cert-file /etc/letsencrypt/archive/dexcelerate.com/cert1.pem  --tls-key-file /etc/letsencrypt/archive/dexcelerate.com/privkey1.pem  --tls-ca-cert-file /etc/letsencrypt/archive/dexcelerate.com/fullchain1.pem
    fi
  fi
}
